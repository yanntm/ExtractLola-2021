...
search
*/
bool EUExploration::depth_first(SimpleProperty &phiproperty, SimpleProperty &psiproperty,NetState &ns, Store<void> &myStore,
                                 Firelist &myFirelist, int)
{
    ...
    while (true) // exit when trying to pop from empty stack
    {
	    ...
	    if(psiproperty.value) return true;
