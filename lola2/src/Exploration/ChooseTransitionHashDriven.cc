...
ChooseTransitionHashDriven::ChooseTransitionHashDriven(Petrinet * n) :
    table(new int[SIZEOF_MARKINGTABLE])
{
	net = n;
	memset(table,0,SIZEOF_MARKINGTABLE * sizeof(int));
}
        ...
        arrayindex_t *firelist)
{
    ...
    for (arrayindex_t i = cardfirelist; i > 0;)
    {
        ...
        hash_t h = (ns.HashCurrent + net->DeltaHash[t]) % SIZEOF_MARKINGTABLE;
    ...
    hash_t h = (ns.HashCurrent + net->DeltaHash[chosen]) % SIZEOF_MARKINGTABLE;
    h = (h < 0) ? h + SIZEOF_MARKINGTABLE : h;
    ++(table[h]);
