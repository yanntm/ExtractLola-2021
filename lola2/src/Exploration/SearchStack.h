...
public:
    ...
    */
    ...
    // Enable traversal of stack, starting at top
    arrayindex_t traversal_stackpointer;
    Chunk<T> * traversal_chunk;

    /// init traversal, return top of stack
    T & init_traversal()
    {
	    traversal_chunk = currentchunk;
	    traversal_stackpointer = StackPointer;
	    return currentchunk->content[(StackPointer - 1) % SIZEOF_STACKCHUNK];
    }
    T & next_traversal()
    {
	    traversal_stackpointer--;
	    if ((traversal_stackpointer % SIZEOF_STACKCHUNK) == 0)
	    {
		// need to jump to previous chunk
		traversal_chunk = traversal_chunk->prev;
	    }
	    return traversal_chunk->content[(traversal_stackpointer - 1) % SIZEOF_STACKCHUNK];
    }
