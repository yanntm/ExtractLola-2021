...
NetState *ParallelExploration::threadedExploration(threadid_t threadNumber)
{
    ...
    Firelist *local_firelist = global_baseFireList->createNewFireList(net,local_property);
    ...
    while (true) // exit when trying to pop from empty stack
    {
        ...
        if (currentEntry-- > 0)
        {
            ...
            net->fire(local_netstate, currentFirelist[currentEntry]);

            if (global_store->searchAndInsert(local_netstate, 0, threadNumber))
            {
                // State exists! --> backtrack to previous state and try again
                net->backfire(local_netstate, currentFirelist[currentEntry]);
                continue;
            }
            // State does not exist!

            // check whether the new state fulfills the property
            net->updateEnabled(local_netstate, currentFirelist[currentEntry]);
            ...
            if (currentEntry >= 2 && num_suspended > 0)
            {
                ...
                if (num_suspended > 0)
                {
                    ...
                    net->backfire(local_netstate, currentFirelist[currentEntry]);
                    net->revertEnabled(local_netstate, currentFirelist[currentEntry]);
        ...
        else
        {
            ...
            local_firelist = global_baseFireList->createNewFireList(net,local_property);
...
bool ParallelExploration::sweepline(SimpleProperty &property, NetState &ns,
                                    SweepEmptyStore &myStore, Firelist &myFirelist, int frontNumber, threadid_t threadNumber, Mara * mem)
{
    Sweep<void> s(net,property, ns, myStore, myFirelist, frontNumber, threadNumber,mem);
    return s.runThreads();
}
