 ...
 *        from Koch.
 */
FirelistStubbornLTL::FirelistStubbornLTL(Petrinet * n,BuechiAutomata *bauto) : bauto(bauto),
...
//    " phi_i UP sets and from the semi-invisibility check.");
    ...
    for (arrayindex_t firstUnprocessed = 0; firstUnprocessed < stackPointer; ++firstUnprocessed)
    {
        ...
        {
            ++cardEnabled;
            mustbeincluded = net->Conflicting[currenttransition];
            cardmustbeincluded = net->CardConflicting[currenttransition];
        }
        else
        {
	    ...
	    arrayindex_t scapegoat = Firelist::selectScapegoat(ns,currenttransition);
            ...
            cardmustbeincluded = net->PlIncreasing[scapegoat];
        }
...
Firelist *FirelistStubbornLTL::createNewFireList(Petrinet * n, BuechiAutomata *bauto)
{
    return new FirelistStubbornLTL(n, bauto);
}
