...
inline void searchAndInsertAutomataState(int automataState,
        ...
        AutomataTree *currentStateEntry)
{
    ...
    if (currentStateEntry->firelist != 0)
    {
        *currentFirelist = currentStateEntry->firelist;
        deadlock = (*currentFirelistEntry = (currentStateEntry->cardFirelist - 1) ) == (int) - 1;
    }
    else
    {
        ...
        AutomataTree *currentStateEntry)
{
    ...
    if (currentFirelistEntry != (int) - 1)
    {
        ...
        {
            if (assumptions.weak_backlist[currentFirelist[i]] != (int) - 1)
            {
                __enabled_weak[assumptions.weak_backlist[currentFirelist[i]]] = true;
            }
            else if (assumptions.strong_backlist[currentFirelist[i]] != (int) - 1)
            {
    ...
    while (true) // exit when trying to pop from empty stack
    {
        ...
        if (currentFirelistEntry != (int) - 1)
        {
            ...
            if (nextStateEntry->dfs == -depth || nextStateEntry->dfs == -depth - 1)
            {
                ...
                if (assumptions.weak_backlist[currentFirelist[currentFirelistEntry]] != (int) - 1)
                {
                ...
                else if (assumptions.strong_backlist[currentFirelist[currentFirelistEntry]] != (int) - 1)
                {
                                ...
                                dfsnum_t depth, arrayindex_t initialDFS)
{
    ...
    while (true) // exit when trying to pop from empty stack
    {
        ...
        if (currentFirelistEntry != (int) - 1 && currentStateListEntry != (int) - 1)
        {
            // there is a next transition that needs to be explored in current marking
                ...
                if (foundcounterexample && nonTrivial)
                {
                    ...
                    if (checkResult == (int) - 1)
                    {
                        ...
                        for (arrayindex_t i = 0; i < ns.CardEnabled; i++)
                            if (assumptions.weak_backlist[ns.Enabled[i]] != (int) - 1)
                            {
