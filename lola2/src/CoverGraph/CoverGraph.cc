...
*/
CoverGraph::CoverGraph(Petrinet * n, SimpleProperty &property, NetState &ns, Store<CoverPayload> &st,
                       Firelist &firelist,
...
void *CoverGraph::coverThread(void *container)
{
    ...
    Firelist *myFirelist = cg->fl.createNewFireList(net,sp);
    // obtain a firelist generator that delivers all enabled transitions (AGEF only)
    Firelist simpleFirelist(net);
    ...
    while (!cg->exit)
    {
        ...
        for (arrayindex_t i = 0; i < currentEntry; ++i)
        {
            ...
            else
            {
                ...
                net->updateEnabled(*ns, currentFirelist[i]);
                ...
                CoverGraph::backfire(*ns, currentFirelist[i]);
                net->revertEnabled(*ns, currentFirelist[i]);
            }
        ...
        while (ipay != cousin)
        {
            ...
            else
            {
                ...
                net->revertEnabled(*ns, ipay->transition);
                ipay = ipay->parent;
            }
...
void CoverGraph::fire(NetState &ns, arrayindex_t t)
{
    ...
    for (arrayindex_t i = 0; i < net->CardDeltaT[PRE][t]; i++)
    {
        ...
        assert(ns.Current[net->DeltaT[PRE][t][i]] >= net->MultDeltaT[PRE][t][i]);

        // omega entries must remain unchanged
        if (ns.Current[net->DeltaT[PRE][t][i]] < OMEGA)
        {
            ns.Current[net->DeltaT[PRE][t][i]] -= net->MultDeltaT[PRE][t][i];
	}
    }
    for (arrayindex_t i = 0; i < net->CardDeltaT[POST][t]; i++)
    {
        // omega entries must remain unchanged
        if (ns.Current[net->DeltaT[POST][t][i]] < OMEGA)
        {
            ns.Current[net->DeltaT[POST][t][i]] += net->MultDeltaT[POST][t][i];
        }
    }

    // 2. update hash value
    ns.HashCurrent += net->DeltaHash[t];
...
void CoverGraph::backfire(NetState &ns, arrayindex_t t)
{
    ...
    for (arrayindex_t i = 0; i < net->CardDeltaT[POST][t]; i++)
    {
        ...
        assert(ns.Current[net->DeltaT[POST][t][i]] >= net->MultDeltaT[POST][t][i]);
...
void CoverGraph::removeOmega(NetState &ns, arrayindex_t t, arrayindex_t p, capacity_t tokens)
{
    ...
    {
        const arrayindex_t tt = net->TrDecreased[t][i];
        if (ns.Enabled[tt])
        {
            net->checkEnabled(ns, tt);
        }
                                      ...
                                      arrayindex_t threadID)
{
    ...
    while (true)
    {
        ...
        for (arrayindex_t i = 0; i < net->CardDeltaT[POST][t]; ++i)
        {
            ...
            arrayindex_t n(net->MultDeltaT[POST][t][i]);
            if (sign[p] >= 0)
            {
                diff[p] += n;
                if (sign[p] == 0)
        ...
        for (arrayindex_t i = 0; i < net->CardDeltaT[PRE][t]; ++i)
        {
            ...
            arrayindex_t n(net->MultDeltaT[PRE][t][i]);
            if (sign[p] <= 0)
            {
                diff[p] += n;
                if (sign[p] == 0)
...
NetState *CoverGraph::getWitnessState(CoverPayload *pay)
{
    ...
    while (!payloads.empty())
    {
        ...
        // update enabledness
        net->updateEnabled(*ns, payloads.back()->transition);
