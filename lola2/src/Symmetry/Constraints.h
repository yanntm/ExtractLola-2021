...
public:
    ...
    SymmetryCalculator(Petrinet *);

    /// construct a calculator for a CTL formula
    explicit SymmetryCalculator(Petrinet * n,  CTLFormula *);

    /// construct a calculator for a state predicate
    explicit SymmetryCalculator(Petrinet * n, StatePredicate *);
...
private:
    ...
    bool initialSplit(Constraint *);

    /// general split of a constraint (on both arrays)
    static bool split(arrayindex_t, Constraint *);
    ...
    static bool ExploreSymmetry(Petrinet *, arrayindex_t);
    static bool ExploreSymmetryForEq(Petrinet *, arrayindex_t);

    /// debug output of constraints
    void printConstraints(int);
    ...
    static void *ExploreSymmetryThreadForEquivalence(void *);
