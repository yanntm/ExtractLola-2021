...
*/
GeneratingSet::GeneratingSet(Petrinet * n) :
    possibleGenerators(((n->Card[PL]) * (n->Card[PL] - 1)) / 2),
    knownGenerators(0),
    realComputed(0),
    store(static_cast<arrayindex_t ** *>(calloc(n->Card[PL], SIZEOF_VOIDP))),
    cardLevels(0),
    depth(static_cast<arrayindex_t *>(calloc(n->Card[PL], SIZEOF_ARRAYINDEX_T))),
    card(static_cast<arrayindex_t *>(calloc(n->Card[PL], SIZEOF_ARRAYINDEX_T))),
    reserve_marking(new capacity_t [n->Card[PL]])
{
	net = n;
