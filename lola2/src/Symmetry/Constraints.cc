...
int SymmetryCalculator::deadBranches = 0;
...
SymmetryCalculator::SymmetryCalculator(Petrinet * n) : F(NULL), S(NULL)
{
	net = n;
	wantequivalenceclasses = false;
}

SymmetryCalculator::SymmetryCalculator(Petrinet * n, CTLFormula * FF) : F(FF), S(NULL)
{
	net = n;
	wantequivalenceclasses = false;
}

SymmetryCalculator::SymmetryCalculator(Petrinet * n,  StatePredicate * SP) : F(NULL), S(SP)
{	
	net = n;
	wantequivalenceclasses = false;
}
...
void *SymmetryCalculator::report_progress(void *)
{
    ...
    while (true)
    {
        ...
        RT::rep->status("%8u generators;%8u candidates remain (%2u%%), %5d secs",
...
void SymmetryCalculator::ComputeSymmetries(int portfolio_id)
{
    // announce start of computation

    portfoliomanager::taskjson[portfolio_id]["search"]["symmetry"]["timelimit"] = JSON::null;
    if (RT::args.symmtimelimit_given)
    {
        portfoliomanager::taskjson[portfolio_id]["search"]["symmetry"]["timelimit"] = RT::args.symmtimelimit_arg;
        portfoliomanager::taskjson[portfolio_id]["search"]["symmetry"]["interrupted"] = false;
    }
    ...
    if (stop)
    {
	portfoliomanager::taskjson[portfolio_id]["search"]["symmetry"]["interrupted"] = true;
    }

    portfoliomanager::taskjson[portfolio_id]["search"]["symmetry"]["generators"] = static_cast<int>(G->knownGenerators);
    portfoliomanager::taskjson[portfolio_id]["search"]["symmetry"]["dead_branches"] = static_cast<int>(deadBranches);
    portfoliomanager::taskjson[portfolio_id]["search"]["symmetry"]["card"] = static_cast<double>(G->sizeofSymmetryGroup());
    portfoliomanager::taskjson[portfolio_id]["search"]["symmetry"]["real_computed"] = static_cast<int>(G->realComputed);
    portfoliomanager::taskjson[portfolio_id]["search"]["symmetry"]["composed"] = static_cast<int>(G->knownGenerators-G->realComputed);
}
...
void SymmetryCalculator::sort(arrayindex_t thr, int direction, arrayindex_t from, arrayindex_t to)
{
    ...
    int pivot = elements[thr][direction][from]->property[direction][thr];
    while (red >= white)
    {
...
void SymmetryCalculator::ExploreLevel()
{
    arrayindex_t startlevel = 0;
    while(true)
    {
	    ...
	    pthread_create(&my_thread, NULL, (wantequivalenceclasses ? ExploreSymmetryThreadForEquivalence : ExploreSymmetryThread ), reinterpret_cast<void *>(MyTicket));
...
bool SymmetryCalculator::ExploreSymmetry(Petrinet * net, arrayindex_t thr)
{
...
begin:
    ...
    // look for place constraint with higher priority
    for (c_min = 0; c_min < cardConstraints[thr]; c_min++)
    {
        if (constraints[thr][c_min].from != constraints[thr][c_min].to && elements[thr][DOM][constraints[thr][c_min].from]->id <= net -> Card[PL])
        {
            // There exists a non-singleton place constraint
            break;
        }
    }
    if (c_min >= cardConstraints[thr])
    {
        // There is no non-singleton place constraint
        // --> try another
RT::rep->status("NO PLACE CONSTRAINT");
	    for (c_min = 0; c_min < cardConstraints[thr]; c_min++)
	    {
...
bool SymmetryCalculator::ExploreSymmetryForEq(Petrinet * net, arrayindex_t thr)
{
    // 1. refine until nothing changes
start:
...
void SymmetryCalculator::printConstraints(int th)   // th is thread number
{
    for (arrayindex_t c = 0; c < cardConstraints[th]; c++)
    {
        std::cout << std::endl << "==========================================" << std::endl;
        for (arrayindex_t j = constraints[th][c].from; j <= constraints[th][c].to; j++)
        {
            arrayindex_t e = elements[th][DOM][j]->id;
            if (e < net->Card[PL])
            {
        ...
        for (arrayindex_t j = constraints[th][c].from; j <= constraints[th][c].to; j++)
        {
            arrayindex_t e = elements[th][COD][j]->id;
           if (e < net->Card[PL])
            {
                std::cout << net->Name[PL][e] << " ";
            }
...
void SymmetryCalculator::printInitialConstraints()
{
    ...
    {
        std::cout << "==========================================" << std::endl;
        for (arrayindex_t j = constraints[0][c].from; j <= constraints[0][c].to; j++)
        {
            arrayindex_t e = elements[0][DOM][j]->id;
            int co = elements[0][DOM][j]->color;
            if (e < net->Card[PL])
            {
                std::cout << net->Name[PL][e];
            }
...
void SymmetryCalculator::printInitialElements()
{
    for (arrayindex_t i = 0; i < net->Card[PL] + net->Card[TR] + sizeFormula; i++)
    {
        if (i < net->Card[PL])
        {
            std::cout << net->Name[PL][elements[0][DOM][i]->id];
        }
        if (i >= net->Card[PL] && i < net->Card[PL] + net->Card[TR])
        {
            std::cout << net->Name[TR][elements[0][DOM][i]->id - net->Card[PL]];
        }
        if (i >= net->Card[PL] + net->Card[TR])
        {
            std::cout << elements[0][DOM][i]->id;
        }
        std::cout << " " << elements[0][DOM][i]->property[DOM] << std::endl;
    }
}
void SymmetryCalculator::printElements(int th)
{
    for (arrayindex_t i = 0; i < net->Card[PL] + net->Card[TR] + sizeFormula; i++)
    {
        if (i < net->Card[PL])
        {
            std::cout << net->Name[PL][elements[th][DOM][i]->id];
        }
        if (i >= net->Card[PL] && i < net->Card[PL] + net->Card[TR])
        {
            std::cout << net->Name[TR][elements[th][DOM][i]->id - net->Card[PL]];
        }
        if (i >= net->Card[PL] + net->Card[TR])
        {
            std::cout << elements[th][DOM][i]->id;
        }
        std::cout << " " << elements[th][DOM][i]->property[DOM] << std::endl;
    }
    std::cout << std::endl << "------------" << std::endl;
    for (arrayindex_t i = 0; i < net->Card[PL] + net->Card[TR] + sizeFormula; i++)
    {
        if (i < net->Card[PL])
        {
            std::cout << net->Name[PL][elements[th][COD][i]->id];
        }
        if (i >= net->Card[PL] && i < net->Card[PL] + net->Card[TR])
        {
            std::cout << net->Name[TR][elements[th][COD][i]->id - net->Card[PL]];
        }
        if (i >= net->Card[PL] + net->Card[TR])
        {
            std::cout << elements[th][COD][i]->id;
        }
        std::cout << " " << elements[th][COD][i]->property[COD] << std::endl;
    }
}
...
void SymmetryCalculator::printGraph(int th)
{
  std::cout << std::endl;
  for(arrayindex_t i = 0; i < net->Card[PL]+net->Card[TR]+sizeFormula;i++)
  {
      Vertex * vv = elements[th][DOM][i];
      // print node
      printId(vv->id);
...
void *SymmetryCalculator::ExploreSymmetryThreadForEquivalence(void *arg)
{
	// same as above, but we are just interested in equivalence classes
	// of places and transitions, not in the generating set as such
    ...
    while (true)  // for all images do...
    {
        ...
        for (arrayindex_t i = ourConstraint->from; i <= ourConstraint->to; i++)
        {
            ...
            if (current_id > previous_image  //not yet processed
                    && current_id < next_id  // smaller than other
                    // candidates
                    && (portfoliomanager::symmfind(PL,T->depth)!= portfoliomanager::symmfind(PL,current_id)))  // no generator known
            {
                next_index = i;
                next_id = current_id;
            }
        ...
        // dive down to find a generator
        if (ExploreSymmetryForEq(net,thr))
        {
            // add symmetry to G
            for (arrayindex_t i = T->depth; i < net->Card[PL]; i++)
            {
		portfoliomanager::symmunion(PL, elements[thr][DOM][i]->id,elements[thr][COD][i]->id);
            }
	    // transition classes may be not fully explored due to parallel transitions
	    // need to unify all elements in constraint

	    for(arrayindex_t i = 0; i < cardConstraints[thr];i++)
	    {
		// skip place constraints
  		if(elements[thr][DOM][constraints[thr][i].from]->id < net -> Card[PL]) continue;

		int pivot = elements[thr][DOM][constraints[thr][i].from]->id - net -> Card[PL];
		for(int j = constraints[thr][i].from + 1; j < constraints[thr][i].to;j++)
		{
			portfoliomanager::symmunion(TR, pivot, elements[thr][DOM][j]->id - net->Card[PL]);
		}
		for(int j = constraints[thr][i].from; j < constraints[thr][i].to;j++)
		{
			portfoliomanager::symmunion(TR, pivot, elements[thr][COD][j]->id - net->Card[PL]);
		}
            }
	    portfoliomanager::reportsymmetry();
        }
    }
