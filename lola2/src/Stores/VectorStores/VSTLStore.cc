...
VSTLStore<void>::VSTLStore(Petrinet * n, Mara * mem, threadid_t num_threads)
{
    this->net = n;
...
bool VSTLStore<void>::searchAndInsert(const capacity_t *in, bitarrayindex_t bitlen,
 ...
 */
bool VSTLStore<void>::popVector(capacity_t *&out, threadid_t threadIndex)
{
    pthread_rwlock_wrlock(&rwlock);
    ...
    if (result)
    {
        std::vector<capacity_t> &ci = intermediate[threadIndex] = *store.begin(); // get first vector
