...
{
private:
    inline int hash_sdbm(const capacity_t *in, size_t len) const;
    inline int hash_fnv(const capacity_t *in, size_t len) const;
...
public:
	Petrinet * net;
    /// constructor
    explicit VBloomStore(Petrinet * n,Mara * mem, threadid_t num_threads, size_t = 2);
    /// destructor
    ~VBloomStore();
    ...
    bool searchAndInsert(const capacity_t *in, bitarrayindex_t bitlen, hash_t hash, void **payload,
