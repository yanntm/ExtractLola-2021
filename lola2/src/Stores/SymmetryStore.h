...
public:
    ~SymmetryStore();
    SymmetryStore(Petrinet * n, Mara * mem, threadid_t number_of_threads, Store<T> *actualStore);
...
template <typename T>
SymmetryStore<T>::SymmetryStore(Petrinet * n, Mara * mem,threadid_t number_of_threads, Store<T> *actualStore):
    Store<T>(n, mem,number_of_threads), actualStore(actualStore)
{
    G = NULL;
}
        ...
        bool noinsert)
{
    ...
    for (arrayindex_t i = 0; i < this->net->Card[PL]; i++)
    {
