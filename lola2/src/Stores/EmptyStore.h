...
public:
    ...
    explicit EmptyStore(Petrinet * n, Mara * mem, threadid_t number_of_threads);
    bool searchAndInsert(NetState &ns, T **payload, threadid_t thread, bool noinsert = false);
};

template <typename T>
EmptyStore<T>::EmptyStore(Petrinet * n, Mara * mem, threadid_t number_of_threads) : Store<T>(n,mem,number_of_threads), tries(0)
{
}
