...
public:
    ...
    explicit SimpleCompressedEncoder(Petrinet * n, int numThreads);
...
private:
    ...
    void addToInput(capacity_t val, bitarrayindex_t numbits, unsigned int *&pInput,
...
class LTLSimpleCompressedEncoder : public NetStateEncoder
{
public:
    /// constructor; initialize auxiliary data structure (namely "inputs").
    /// @param numThreads maximum number of threads that may work with this NetStateEncoder concurrently.
    explicit LTLSimpleCompressedEncoder(Petrinet * n, int numThreads);

    /// destructor; frees all memory used for auxiliary data structure (namely "inputs").
    ~LTLSimpleCompressedEncoder();
