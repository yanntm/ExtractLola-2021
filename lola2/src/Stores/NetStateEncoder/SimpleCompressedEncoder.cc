...
SimpleCompressedEncoder::SimpleCompressedEncoder(Petrinet * n, int numThreads) : NetStateEncoder(n,numThreads)
{
    // calculate worst case input vector length (assuming that all markings need maximal length)
    insize = ((2 * net->CardSignificant * PLACE_WIDTH + VECTOR_WIDTH - 1) / VECTOR_WIDTH) * sizeof(
                 capacity_t);
        ...
        bitarrayindex_t &input_bitstogo)
{
    ...
    while (numbits)
    {
        ...
        *pInput |= capacity_t((capacity_t(val << (PLACE_WIDTH - numbits)) >>
...
unsigned int *SimpleCompressedEncoder::encodeNetState(NetState &ns, bitarrayindex_t &bitlen,
        arrayindex_t threadIndex)
{
    ...
    capacity_t *pInput = pCurThreadInput;
    ...
    for (arrayindex_t place_index = 0; place_index < net->CardSignificant; place_index++)
    {
...
LTLSimpleCompressedEncoder::LTLSimpleCompressedEncoder(Petrinet * n, int numThreads) : NetStateEncoder(n,numThreads)
{
    // calculate worst case input vector length (assuming that all markings need maximal length)
    insize = ((2 * (net->CardSignificant+1) * PLACE_WIDTH  + VECTOR_WIDTH - 1) / VECTOR_WIDTH) * sizeof(
                 capacity_t);
...
LTLSimpleCompressedEncoder::~LTLSimpleCompressedEncoder()
{
    // free auxiliary input vectors.
    for (int i = 0; i < numThreads; i++)
    {
...
void LTLSimpleCompressedEncoder::addToInput(capacity_t val, bitarrayindex_t numbits,
        unsigned int *&pInput,
        bitarrayindex_t &input_bitstogo)
{
    // repeat as long as there are bits to copy (may happen when remaining bits of current input vector word are not sufficient to hold all the bits)
...
unsigned int *LTLSimpleCompressedEncoder::encodeNetState(NetState &ns, bitarrayindex_t &bitlen,
        arrayindex_t threadIndex)
{
    // initialize number of bits. Will be increased as bits are added to the input vector.
    bitlen = 0;
    ...
    for (arrayindex_t place_index = 0; place_index < net->CardSignificant; place_index++)
    {
        ...
        // current marking value to store
        capacity_t curMarking = ns.buchistate;
