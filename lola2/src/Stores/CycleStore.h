...
public:
    ~CycleStore();
    CycleStore(Petrinet * n, Mara * mem, threadid_t number_of_threads, Store<T> *actualStore, int k);
...
template <typename T>
CycleStore<T>::CycleStore(Petrinet * n, Mara * mem, threadid_t number_of_threads, Store<T> *actualStore, int k) :
    Store<T>(n,mem,number_of_threads), actualStore(actualStore),
    u(new bool[n->Card[TR]]), k(k)
{
