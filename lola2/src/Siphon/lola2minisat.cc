...
void create_siphon(Petrinet * net, SimpSolver &S)
{
...
void create_post(Petrinet * net, SimpSolver &S, arrayindex_t t, arrayindex_t i)
{
...
void create_step(Petrinet * net, SimpSolver &S, arrayindex_t i)
{
...
void create_pre(Petrinet * net, SimpSolver &S, arrayindex_t t)
{
	// --> part of <-> is single clause
...
void create_unmarked(Petrinet * net, SimpSolver &S)
{
...
void create_notrap(Petrinet * net, SimpSolver &S)
{
...
void create_notrap_t(Petrinet * net, SimpSolver &S, arrayindex_t t)
{
...
void create_eval(Petrinet * net, SimpSolver &S)
{
	// actual eval formula
	vec<Lit> eval;
	...
	create_unmarked(net,S);
	create_notrap(net,S);
	for(arrayindex_t t = 0; t < net->Card[TR];t++)
	{
		create_pre(net,S,t);
		create_notrap_t(net,S,t);
	}
}
...
void create_whole(Petrinet * net, SimpSolver &S)
{
	...
	for(arrayindex_t t = 0; t < net->Card[TR];t++)
	{
		for(arrayindex_t i = 0; i <= SiphonDepth; i++)
		{
			create_post(net,S,t,i);
		}
	}
}

siphon_result_t run_minisat(Petrinet *, int argc,char ** argv, int portfolio_id);

siphon_result_t lola2minisat(Petrinet * net, int portfolio_id)
{
	if(net -> Card[TR] == 0) 
	{
	...
	if(RT::args.check_arg == check_arg_deadlockfreedom)
	{
		...
		if((res = run_minisat(net, argcc, argvv, portfolio_id)) == SIPHON_PROPERTY_TRUE)
		{
			RT::rep->status(RT::rep->markup(MARKUP_GOOD,"No deadlock is reachable.").str());
                        portfoliomanager::compareresult(false);
                        if(RT::args.mcc_given)
	...
	{
		return run_minisat(net, argcc,argvv, portfolio_id);
	}
}
...
siphon_result_t run_minisat(Petrinet * net,int argc,char ** argv, int portfolio_id)
{
    try {
...
void printAssignment(Petrinet * net, SimpSolver & S)
{
	// unmarked
