...
//TODO extern void yyerrors(char* token, char const* format, ...);
extern char* yytext;
...
UNumType::UNumType(int low, int up) {
    if (low > up) {
        //TODO yyerrors(yytext, "negative range in integer type definition");
    }
    lower = low;
    upper = up;
    size = up - low + 1;
...
//    yyerrors(yytext, "runtime error: %s", _cimportant_(mess)); TODO
}
