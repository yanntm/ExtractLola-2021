...
*/
void PlaceSymbol::addInitialMarking(capacity_t newTokens)
{
...
/*
PlaceSymbol * PlaceSymbol::copy()
{
	PlaceSymbol * result = new PlaceSymbol(getKey(),getCapacity());
	result -> initialMarking = initialMarking;
	result -> cardPost = cardPost;
	result -> cardPre = cardPre;
	result -> nestedunit = nestedunit;
	result -> visible = 0;
