...
*/
TransitionSymbol::TransitionSymbol(const char *k, fairnessAssumption_t f = NO_FAIRNESS,
                                   ArcList *pr = NULL , ArcList *po = NULL) :
    Symbol(k),
    fairness(f),
...
/*
TransitionSymbol * TransitionSymbol::copy()
{
	TransitionSymbol * result = new TransitionSymbol(getKey(),fairness);
	result -> visible = 0;
	result -> inEmptySiphon = inEmptySiphon;
	result -> index = index;
	result -> next = 0;
	result -> cardPre = cardPre;
	result -> cardPost = cardPost;
