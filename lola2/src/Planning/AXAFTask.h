...
class AXAFTask : public Task
{
public:
    ~AXAFTask(){}
     AXAFTask(Petrinet * n, int par, tFormula  f, int fid)
     {
	preprocessingfinished = false;
	net = n;
	parent = par;
	taskname = deconst("AXAF");
	formula = f;
	formula_id = fid;
	result = TERNARY_UNKNOWN;
	portfolio_id = portfoliomanager::addTask(n,this,par,fid,FORMULA_AXAF, DUAL_TASK);
     }
    ...
    switch (result)
    {
    case TERNARY_TRUE: return deconst("The predicate eventually occurs from all successors.");
    case TERNARY_FALSE: return deconst("The predicate does not eventually occur from all successors.");
    case TERNARY_UNKNOWN: return deconst("The predicate may or may not eventually occur from successors.");
        break;
...
return NULL;
	...
	AXAFTask * N = new AXAFTask(n,par,f,fid);
	f->id = NULL;
	int my_id = portfoliomanager::addFormula(f);
	EXEGTask::buildTask(n,N->portfolio_id,f,my_id);
