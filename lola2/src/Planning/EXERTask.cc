...
This class executes the EXER check by statespace exploration

*/


EXERTask::EXERTask(Petrinet * n, int par, tFormula  f, int fid)
{
    ...
    memory = new Mara();
    taskname = deconst("state space / EXER");
    portfolio_id = portfoliomanager::addTask(n,this,par,fid,FORMULA_EXER,EXCL_MEM_TASK,memory);
    goStatus = false;
...
ternary_t EXERTask::getResult()
{
    ...
    exploration = new EXERExploration(net);
...
char * EXERTask::interpreteResult(ternary_t result)
{
    switch (result)
    {
    case TERNARY_TRUE: return deconst("There is a path where Predicate Phi releases Predicate Psi from some successor.");
    case TERNARY_FALSE: return deconst("There is no path where Predicate Phi releases Predicate Psi from any successor.");
    case TERNARY_UNKNOWN: return deconst("There may or may not be a path where Predicate Phi releases Predicate Psi from successor.");
    default: assert(false);
    }
	return NULL;
}

Path EXERTask::getWitnessPath()
{
    // simple property
    return phi->path();
}

capacity_t *EXERTask::getMarking()
{
...
statusrecord * EXERTask::getStatistics()
{
...
void EXERTask::buildTask(Petrinet * n,int par, tFormula  f, int fid)
{
    if(!RT::args.quickchecks_given)
        {
            EXERTask * E = new EXERTask(n,par,f,fid);
	   ...
	   return;
        ...
        //   Quickcheck(suff)         EXERTask
        //       |ag1
        //   Invariant
        //       |ag2
        //   Stateequation
        //
        int agg = portfoliomanager::addTask(n,NULL, par, fid, FORMULA_EXER,AGGREGATE_TASK);
        ...
        EXERTask * E = new EXERTask(n,agg,liveformula,live_id);
	E -> preprocessingfinished = true;
...
statusrecord  *EXERTask::getStatus()
{
    if (!goStatus)
    {
...
Task * EXERTask::copy()
{
	return new EXERTask(net,parent,portfoliomanager::copyFormula(formula),formula_id);
}
