...
This class executes the EXEU check by statespace exploration

*/


EXEUTask::EXEUTask(Petrinet * n, int par, tFormula  f, int fid)
{
    ...
    memory = new Mara();
    taskname = deconst("state space / EXEU");
    portfolio_id = portfoliomanager::addTask(n,this,par,fid,FORMULA_EXEU,EXCL_MEM_TASK,memory);
    goStatus = false;
...
ternary_t EXEUTask::getResult()
{
    ...
    exploration = new EXEUExploration(net);
...
char * EXEUTask::interpreteResult(ternary_t result)
{
    switch (result)
    {
    case TERNARY_TRUE: return deconst("There is a path where Predicate Phi holds until Predicate Psi from some successor.");
    case TERNARY_FALSE: return deconst("There is no path where Predicate Phi holds until Predicate Psi from any successor.");
    case TERNARY_UNKNOWN: return deconst("There may or may not be a path where Predicate Phi holds until Predicate Psi from successors.");
    default: assert(false);
    }
	return NULL;
}

Path EXEUTask::getWitnessPath()
{
    // simple property
    return phi->path();
}

capacity_t *EXEUTask::getMarking()
{
...
statusrecord * EXEUTask::getStatistics()
{
...
void EXEUTask::buildTask(Petrinet * n, int par,tFormula  f, int fid)
{
    if (!RT::args.quickchecks_given)
    {
        EXEUTask * E = new EXEUTask(n,par,f,fid);
	...
	return;
        ...
        //   Quickcheck(necc)         EXEUTask
        //       |ef1
        //   Stateequation
        //   
        int agg = portfoliomanager::addTask(n,NULL, par, fid, FORMULA_EXEU,AGGREGATE_TASK);
        ...
        EXEUTask * E = new EXEUTask(n,agg,liveformula,live_id);
	E -> preprocessingfinished = true;
...
statusrecord *EXEUTask::getStatus()
{
    if (!goStatus)
    {
...
Task * EXEUTask::copy()
{
	return new EXEUTask(net,parent,portfoliomanager::copyFormula(formula),formula_id);
}
