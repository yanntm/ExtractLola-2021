...
This class delegates the SINGLEPATHEX problem to the SINGLEPATHALL problem.
...
class SinglePathExTask : public Task
{
public:
    ~SinglePathExTask(){}
    SinglePathExTask(Petrinet * n, int par, tFormula f, int fid)
    {
	...
	taskname = deconst("existential single path");
	portfolio_id = portfoliomanager::addTask(n,this,par,fid,FORMULA_SINGLEPATHEX,DUAL_TASK);
    }
    ...
    switch (result)
    {
    case TERNARY_TRUE: return deconst("The existential single path CTL formula holds.");
    case TERNARY_FALSE: return deconst("The existential single path CTL formula does not hold.");
    case TERNARY_UNKNOWN: return deconst( "The existential single path CTL formula may or may not hold.");
   default: assert(false);
	...
	Task * spath = new SinglePathExTask(n,par,f,fid);
	f -> id = NULL;
	f = StatePredicateFormula(Negation((tStatePredicate) f ->subphylum(0)));
	f = f->rewrite(kc::onlynegation);
	f->unparse(myprinter,kc::temporal);
	int myid = portfoliomanager::addFormula(f);
	SinglePathAllTask::buildTask(n,spath->portfolio_id,f,myid);
	spath -> preprocessingfinished = true;
    }
    void derail(){}
