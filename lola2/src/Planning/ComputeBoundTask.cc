...
ComputeBoundTask::ComputeBoundTask(Petrinet * n,int par, tFormula  f, int fid)
{
	...
	ComputeBoundExploration::lp(net, (AtomicStatePredicate *) spFormula);
...
char * ComputeBoundTask::interpreteResult(ternary_t r)
{
...
statusrecord * ComputeBoundTask::getStatistics()
{
...
void ComputeBoundTask::buildTask(Petrinet * n, int par, tFormula  f, int fid)
{
	ComputeBoundTask * C = new ComputeBoundTask(n,par,f,fid);
	C -> preprocessingfinished = true;
}


statusrecord  * ComputeBoundTask::getStatus()
{
	if(!goStatus)
	{
		return NULL;
...
void ComputeBoundTask::derail()
{
	memory->stop = true;
}

Task * ComputeBoundTask::copy()
{
	return new ComputeBoundTask(net,parent,portfoliomanager::copyFormula(formula),formula_id);
}
