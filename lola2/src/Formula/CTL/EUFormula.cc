...
EUFormula::EUFormula(Petrinet * n, CTLFormula *phi, CTLFormula *psi) : phi(phi), psi(psi)
{
    net = n;
    assert(phi);
    assert(psi);
...
*/
bool EUFormula::check(Store<void *> &s, NetState &ns, Firelist &firelist, Firelist & sfirelist,
                      std::vector<int> &witness)
{
    ...
    // check phi
    if (not phi->check(s, ns, firelist, sfirelist,witness))
    {
