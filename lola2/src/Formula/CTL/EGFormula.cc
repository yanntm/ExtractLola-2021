...
\brief Implementation of the CTL EG operator.
*/
...
EGFormula::EGFormula(Petrinet * n, CTLFormula *psi) : psi(psi)
{
    net = n;
    assert(psi);
}

void EGFormula::initAtomics(NetState &ns)
{
    psi->initAtomics(ns);
}

void EGFormula::updateAtomics(NetState &ns, arrayindex_t t)
{
    psi->updateAtomics(ns, t);
}

void EGFormula::revertAtomics(NetState &ns, arrayindex_t t)
{
    psi->revertAtomics(ns, t);
}

// LCOV_EXCL_START
void EGFormula::DEBUG_print()
{
    printf("[%u,%u,%zu]EG(", index, dfsindex, payloadsize);
    psi->DEBUG_print();
    printf(")");
}
// LCOV_EXCL_STOP

void EGFormula::gatherPayloadInformation(arrayindex_t &numDFS,
...
void EGFormula::setPayloadInformation(arrayindex_t cachedResultOffset,
                                      size_t payloadSize)
{
...
bool EGFormula::check(Store<void *> &s, NetState &ns, Firelist &firelist, Firelist &sfirelist,
...
//RT::rep->status("init EG check at %x (payload: %lx)",ns.HashCurrent,payload);

    CTLFormulaResult cachedResult = getCachedResult(payload);
    if (cachedResult & 2)  // value known
    {
     //   RT::rep->status("EG cached %d at %x",cachedResult,ns.HashCurrent);
    ...
    //    RT::rep->status("EG initial FALSE at %x",ns.HashCurrent);
...
//RT::rep->status("starting EG check at %x (payload: %lx)",ns.HashCurrent,payload);
    ...
    while (true)
    {
//RT::rep->status("MARKING at SP %d",dfsStack.StackPointer);
//for(arrayindex_t i = 0; i < net->Card[PL];i++)
//{
	//if(ns.Current[i] > 0) RT::rep->status("pl %s : %d",net->Name[PL][i],ns.Current[i]);
//}
//RT::rep->status("REMAINING FL");
//for(arrayindex_t i = 0; i < currentFirelistIndex;i++)
//{
	//RT::rep->status("tr %s",net->Name[TR][currentFirelist[i]]);
//}
        if (currentFirelistIndex--)
        {
//RT::rep->status("fire tr %s",net->Name[TR][currentFirelist[currentFirelistIndex]]);
...
//RT::rep->status("EG check fire %s to %x (payload: %lx)",net->Name[TR][currentFirelist[currentFirelistIndex]],ns.HashCurrent,newpayload);
...
//RT::rep->status("EG check backfire (psi) %s",net->Name[TR][currentFirelist[currentFirelistIndex]]);
...
//RT::rep->status("EG check backfire (KNOWN_FALSE) %s",net->Name[TR][currentFirelist[currentFirelistIndex]]);
        ...
        else     // if(currentFirelistIndex--)
        {
            ...
            //{
//RT::rep->status("EG found SCC at %x",ns.HashCurrent);
...
//RT::rep->status("EG check backfire (fl) %s",net->Name[TR][currentFirelist[currentFirelistIndex]]);
...
//RT::rep->status("EG proven FALSE at %x",ns.HashCurrent);
    ...
    if (backfireNeeded)
    {
//RT::rep->status("EG check backfire (backfireNeeded) %s",net->Name[TR][currentFirelist[currentFirelistIndex]]);
    ...
    // current state can reach counterexample state -> formula true
    setCachedResult(payload, KNOWN_TRUE);
...
//RT::rep->status("EG check backfire (dfsStack) %s",net->Name[TR][dfsStack.top().fl[dfsStack.top().flIndex]]);
...
//RT::rep->status("EG proven TRUE at %x",ns.HashCurrent);

    return true;
}

bool EGFormula::checkfair(Store<void *> &s, NetState &ns, Firelist &firelist,
                      std::vector<int> &witness)
{
    void **pInitialPayload;
    if (!s.searchAndInsert(ns, &pInitialPayload, 0))
    ...
    if (!psi->checkfair(s, ns, firelist, witness))
    {
        ...
        //RT::rep->status("ER initial FALSE at %x",ns.HashCurrent);
    ...
    //rep->status("starting EG check at %x (payload: %lx)",ns.HashCurrent,payload);
    ...
    while (true)
    {
        if (currentFirelistIndex--)
        {
            ...
            //rep->status("ER check fire %s to %x (payload: %lx)",net->Name[TR][currentFirelist[currentFirelistIndex]],ns.HashCurrent,newpayload);
            ...
            if (newCachedResult == UNKNOWN)
            {
                ...
                if (!psi->checkfair(s, ns, firelist, witness))
                {
                    ...
                    //rep->status("ER check backfire (psi) %s",net->Name[TR][currentFirelist[currentFirelistIndex]]);
    ...
    firelist.visible = visible;
                ...
                if((currentFirelistIndex > 1) && (currentDFSNumber > firedAll))
		{
            ...
            else if (newCachedResult == IN_PROGRESS)
            {
                ...
                //rep->status("ER check backfire (IN_PROGRESS) %s",net->Name[TR][currentFirelist[currentFirelistIndex]]);
            ...
            else     // KNOWN_FALSE
            {
                ...
                //rep->status("ER check backfire (KNOWN_FALSE) %s",net->Name[TR][currentFirelist[currentFirelistIndex]]);
        ...
        else     // if(currentFirelistIndex--)
        {
            ...
            if (dfs == currentLowlink)
            {
                //rep->status("ER found SCC at %x",ns.HashCurrent);
		...
		if(dfs > firedAll)
		{
			...
			if(ns.CardEnabled > 1)
			{
                ...
                {
                    //rep->status("ER found fair SCC at %x",ns.HashCurrent);
                    // no need to pop elements from Tarjan stack to set them to KNOWN_FALSE, this is done later anyway
                    backfireNeeded = false;
                    break;
                }
                //rep->status("ER found unfair SCC at %x",ns.HashCurrent);
            ...
            if (dfsStack.StackPointer)
            {
                ...
                //rep->status("ER check backfire (fl) %s",net->Name[TR][currentFirelist[currentFirelistIndex]]);
            ...
            else
            {
                ...
                //rep->status("EG proven FALSE at %x",ns.HashCurrent);
    ...
    if (backfireNeeded)
    {
        //rep->status("ER check backfire (backfireNeeded) %s",net->Name[TR][currentFirelist[currentFirelistIndex]]);
    ...
    while (dfsStack.StackPointer)
    {
        ...
        //rep->status("ER check backfire (dfsStack) %s",net->Name[TR][dfsStack.top().fl[dfsStack.top().flIndex]]);
    ...
    //rep->status("ER proven TRUE at %x",ns.HashCurrent);
...
bool EGFormula::getFairWitness(Store<void *> &s, NetState &ns,
...
void EGFormula::constructWitness(Store<void *> &s, NetState &ns,
                                 Firelist &firelist, std::vector<int> &witness,
...
void EGFormula::produceWitness(Store<void *> &s, NetState &ns,
                               Firelist &firelist, std::vector<int> &witness,
...
void EGFormula::findWitnessPathTo(Store<void *> &s, NetState &ns,
                                  Firelist &firelist, std::vector<int> &witness,
...
bool EGFormula::subdivideFairnessCheck(Store<void *> &s, NetState &ns,
                                       Firelist &firelist,
                                       std::vector<int> &witness,
                                       fairness_data &fairness)
{
...
bool EGFormula::fairSCC(Store<void *> &s, NetState &ns,
                        Firelist &firelist, std::vector<int> &witness,
                        fairness_data &fairness)
{
    void **pInitialPayload;
...
FormulaInfo *EGFormula::getInfo() const
{
...
int EGFormula::countSubFormulas() const
{
    return 1 + psi->countSubFormulas();
}

void EGFormula::print()
{
	std::cout << "EG ( ";
	psi->print();
	std::cout << ")";
