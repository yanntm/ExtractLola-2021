...
NotFormula::NotFormula(Petrinet * n, CTLFormula *inner) : inner(inner)
{
    net = n;
    assert(inner);
}
...
bool NotFormula::check(Store<void *> &s, NetState &ns, Firelist &firelist, Firelist & sfirelist,
                       std::vector<int> &witness)
{
    return not inner->check(s, ns, firelist, sfirelist, witness);
}
