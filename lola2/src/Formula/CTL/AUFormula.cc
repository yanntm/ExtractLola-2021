...
AUFormula::AUFormula(Petrinet * n, CTLFormula *phi, CTLFormula *psi) : phi(phi), psi(psi)
{
    net = n;
    assert(phi);
    assert(psi);
...
bool AUFormula::check(Store<void *> &s, NetState &ns, Firelist &firelist, Firelist &sfirelist,
                      std::vector<int> &witness)
{
    ...
    // change: remove tarjan stack since in checkAU all SCC are trivial
    // (cycles are immediate counterexamples)
