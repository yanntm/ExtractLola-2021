...
\brief Implementation of the CTL ER operator.
*/
...
ERFormula::ERFormula(Petrinet * n, CTLFormula *phi, CTLFormula *psi) : phi(phi), psi(psi)
{
    net = n;
    assert(phi);
    assert(psi);
...
void ERFormula::initAtomics(NetState &ns)
{
    phi->initAtomics(ns);
    psi->initAtomics(ns);
}

void ERFormula::updateAtomics(NetState &ns, arrayindex_t t)
{
    phi->updateAtomics(ns, t);
    psi->updateAtomics(ns, t);
}

void ERFormula::revertAtomics(NetState &ns, arrayindex_t t)
{
...
void ERFormula::gatherPayloadInformation(arrayindex_t &numDFS,
...
void ERFormula::setPayloadInformation(arrayindex_t cachedResultOffset,
                                      size_t payloadSize)
{
...
bool ERFormula::check(Store<void *> &s, NetState &ns, Firelist &firelist, Firelist &sfirelist,
                      std::vector<int> &witness)
{
    ...
    //RT::rep->status("init ER check at %x (payload: %lx)",ns.HashCurrent,payload);

    CTLFormulaResult cachedResult = getCachedResult(payload);
    if (cachedResult & 2)  // value known
    {
        //RT::rep->status("ER cached %d at %x",cachedResult,ns.HashCurrent);
    ...
    witness.clear();
    // phi and psi -> E phi R psi
    if (phi->check(s, ns, firelist, sfirelist, witness))
    {
        setCachedResult(payload, KNOWN_TRUE);

        //RT::rep->status("ER initial TRUE at %x",ns.HashCurrent);

        return true;
    }

    //RT::rep->status("starting ER check at %x (payload: %lx)",ns.HashCurrent,payload);
    ...
    while (true)
    {
//net->DEBUG__printMarking(ns.Current);
        if (currentFirelistIndex--)
        {
            ...
            //RT::rep->status("ER check fire %s to %x (payload: %lx)",net->Name[TR][currentFirelist[currentFirelistIndex]],ns.HashCurrent,newpayload);
            ...
            if (newCachedResult == UNKNOWN)
            {
                ...
                if (!psi->check(s, ns, firelist, sfirelist,witness))
                {
                    ...
                    //RT::rep->status("ER check backfire (psi) %s",net->Name[TR][currentFirelist[currentFirelistIndex]]);
    ...
    myfirelist.visible = visible;
                ...
                if((currentFirelistIndex > 1) && (currentDFSNumber > firedAll))
		{
            ...
            else     // KNOWN_FALSE
            {
                ...
                //RT::rep->status("ER check backfire (KNOWN_FALSE) %s",net->Name[TR][currentFirelist[currentFirelistIndex]]);
        ...
        else     // if(currentFirelistIndex--)
        {
            ...
            //{
                //RT::rep->status("ER found SCC at %x",ns.HashCurrent);
          ...
          //          tarjanStack.pop();
           //     }
             ...
             //   {
              //      currentLowlink = dfsStack.top().lowlink;
               // }
                ...
                //RT::rep->status("ER check backfire (fl) %s",net->Name[TR][currentFirelist[currentFirelistIndex]]);
            ...
            else
            {
                ...
                //RT::rep->status("ER proven FALSE at %x",ns.HashCurrent);
    ...
    if (backfireNeeded)
    {
        //RT::rep->status("ER check backfire (backfireNeeded) %s",net->Name[TR][currentFirelist[currentFirelistIndex]]);
    ...
    while (dfsStack.StackPointer)
    {
        ...
        //RT::rep->status("ER check backfire (dfsStack) %s",net->Name[TR][dfsStack.top().fl[dfsStack.top().flIndex]]);
    ...
    //RT::rep->status("ER proven TRUE at %x",ns.HashCurrent);

    return true;
}

bool ERFormula::checkfair(Store<void *> &s, NetState &ns, Firelist &firelist,
                      std::vector<int> &witness)
{
    void **pInitialPayload;
    if (!s.searchAndInsert(ns, &pInitialPayload, 0))
    ...
    //rep->status("starting ER check at %x (payload: %lx)",ns.HashCurrent,payload);
    ...
    firelist.visible = visible;
		...
		if(dfs > firedAll)
		{
            ...
            else
            {
                ...
                //rep->status("ER proven FALSE at %x",ns.HashCurrent);
...
bool ERFormula::getFairWitness(Store<void *> &s, NetState &ns,
...
void ERFormula::constructWitness(Store<void *> &s, NetState &ns,
                                 Firelist &firelist, std::vector<int> &witness,
...
void ERFormula::produceWitness(Store<void *> &s, NetState &ns,
                               Firelist &firelist, std::vector<int> &witness,
...
void ERFormula::findWitnessPathTo(Store<void *> &s, NetState &ns,
                                  Firelist &firelist, std::vector<int> &witness,
...
bool ERFormula::subdivideFairnessCheck(Store<void *> &s, NetState &ns,
                                       Firelist &firelist,
                                       std::vector<int> &witness,
                                       fairness_data &fairness)
{
...
bool ERFormula::fairSCC(Store<void *> &s, NetState &ns,
                        Firelist &firelist, std::vector<int> &witness,
                        fairness_data &fairness)
{
    void **pInitialPayload;
    ...
    firelist.visible = visible;
        ...
        else     // if(currentFirelistIndex--)
        {
            ...
            if (dfs == currentLowlink)
            {
                ...
                // note that KNOWN_FALSE cannot occur in any reachable node (otherwise it would be a counterexample path that would have been found in CheckER)
                while (tarjanStack.StackPointer && getDFS(tarjanStack.top()) > dfs)
                {
                    setCachedResult(tarjanStack.top(), KNOWN_TRUE);
...
FormulaInfo *ERFormula::getInfo() const
{
...
int ERFormula::countSubFormulas() const
{
    return 1 + phi->countSubFormulas() + psi->countSubFormulas();
}

void ERFormula::print()
{
	std::cout << "ER ( ";
	phi -> print();
	std::cout << ", ";
...
void ERFormula::setVisible()
{
	phi->setVisible();
	psi->setVisible();
	visible = new bool [net->Card[TR]];
	for(arrayindex_t i = 0; i < net->Card[TR]; i++)
	{
		visible[i] = phi->visible[i] || psi->visible[i];
	}
}
