...
\brief Implementation of the CTL AR operator.
*/
...
ARFormula::ARFormula(Petrinet * n,CTLFormula *phi, CTLFormula *psi) : phi(phi), psi(psi)
{
	net = n;
    assert(phi);
    assert(psi);
...
void ARFormula::initAtomics(NetState &ns)
{
    phi->initAtomics(ns);
    psi->initAtomics(ns);
}

void ARFormula::updateAtomics(NetState &ns, arrayindex_t t)
{
    phi->updateAtomics(ns, t);
    psi->updateAtomics(ns, t);
}

void ARFormula::revertAtomics(NetState &ns, arrayindex_t t)
{
...
*/
void ARFormula::gatherPayloadInformation(arrayindex_t &numDFS,
...
void ARFormula::setPayloadInformation(arrayindex_t cachedResultOffset,
                                      size_t payloadSize)
{
...
/*!
Implements the check whether A(phi R psi) holds in the current state. The
...
\return Whether A(phi R psi) holds in the current state. In case the formula is
proved false, witness contains a witness path.

\post during the check, sucessor states may be checked recursively
*/
bool ARFormula::check(Store<void *> &s, NetState &ns, Firelist &firelist, Firelist & sfirelist,
                      std::vector<int> &witness)
{
    ...
    //=========================================
    // (3) quick check 1: !psi => !A(phi R psi)
    //=========================================

    // check psi
    if (!psi->check(s, ns, firelist, sfirelist,witness))
    {
        // We know that !psi holds, so !A(phi R psi) holds by definition. We
        // hence found a witness for !A(phi R psi) and return.
    ...
    //======================================================
    // (4) quick check 2: (phi AND psi) => A(phi U psi)
    //======================================================

    // check phi
    if (phi->check(s, ns, firelist, sfirelist,witness))
    {
        // We know that phi and psi hold, so A(phi A psi) holds
        // by definition. We hence could prove A(phi R psi) and return.
    ...
    while (true)
    {
        ...
        if (currentFirelistIndex--)
        {
            ...
            if (newCachedResult == UNKNOWN)
            {
                ...
                //==========================================
                // (6a) quick check 1: !psi => !A(phi R psi)
                //==========================================

                // check psi
                if (!psi->check(s, ns, firelist, sfirelist, witness))
                {
                    // We know that !psi holds, so !A(phi R psi) holds by
                    // definition. We hence found a witness for !A(phi R psi) and
                ...
                //=======================================================
                // (6b) quick check 2: (phi AND psi) => A(phi R psi)
                //=======================================================

                // check phi
                if (phi->check(s, ns, firelist, sfirelist, witness))
                {
                    // We know that neither phi nor psi hold, so we could 
                    // prove A(phi R psi) for this state and hence backtrack
                    // and continue.
    ...
    myfirelist.visible = visible;
		...
		if((currentDFSNumber > firedAll) && (currentFirelistIndex > 1))
		{
            ...
            else if (newCachedResult == KNOWN_FALSE)
            {
                ...
                // found a witness for !A(phi R psi) and can abort the search by
            ...
            else if (newCachedResult == IN_PROGRESS)
            {
                ...
                // in progress. We hence can prove A(phi R psi) yet and
            ...
            else // (newCachedResult == KNOWN_TRUE)
            {
                // The result of the new state is known to be false. We hence
                // could prove A(phi R psi) for this state and hence
		...
		if(dfs > firedAll)
            ...
            {
                //===========================================================
                // (8a) we did not found a witness -> A(phi R psi) is true
                //===========================================================
    ...
    // => A(phi R psi) does not hold - we only need to organize the witness
    ...
    // A(phi R psi) does not hold and we found a witness path
    return false;
}

bool ARFormula::checkfair(Store<void *> &s, NetState &ns, Firelist &firelist,
                      std::vector<int> &witness)
{
    //==================================
    // (1) prepare marking and payload
...
FormulaInfo *ARFormula::getInfo() const
{
...
int ARFormula::countSubFormulas() const
{
    return 1 + phi->countSubFormulas() + psi->countSubFormulas();
}

void ARFormula::print()
{
	std::cout << "AR( ";
	phi -> print();
	std::cout << ", ";
