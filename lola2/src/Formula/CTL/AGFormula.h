...
\brief Definition of the CTL AG operator.
*/
...
class AGFormula : public DFSFormula
{
public:
    AGFormula(Petrinet *, CTLFormula *psi);
    ...
    std::string ctl2string() { return std::string("AG(") + psi -> ctl2string() + std::string(")");}
...
private:
    ...
    /// the first inner formula for AG(psi)
    CTLFormula *psi;
    virtual void print();
    void setVisible();
};
