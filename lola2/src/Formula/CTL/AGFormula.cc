...
\brief Implementation of the CTL AG operator.
*/
...
AGFormula::AGFormula(Petrinet * n, CTLFormula *psi) : psi(psi)
{
    net = n;
    assert(psi);
}

void AGFormula::initAtomics(NetState &ns)
{
    psi->initAtomics(ns);
}

void AGFormula::updateAtomics(NetState &ns, arrayindex_t t)
{
    psi->updateAtomics(ns, t);
}

void AGFormula::revertAtomics(NetState &ns, arrayindex_t t)
{
    psi->revertAtomics(ns, t);
}

// LCOV_EXCL_START
void AGFormula::DEBUG_print()
{
...
*/
void AGFormula::gatherPayloadInformation(arrayindex_t &numDFS,
...
void AGFormula::setPayloadInformation(arrayindex_t cachedResultOffset,
                                      size_t payloadSize)
{
...
/*!
Implements the check whether AG(psi) holds in the current state. The
...
\return Whether AG(psi) holds in the current state. In case the formula is
proved false, witness contains a witness path.

\post during the check, sucessor states may be checked recursively
*/
bool AGFormula::check(Store<void *> &s, NetState &ns, Firelist &firelist, Firelist & sfirelist,
                      std::vector<int> &witness)
{
    ...
    //=========================================
    // (3) quick check 1: !psi => !AG(psi)
    //=========================================

    // check psi
    if (!psi->check(s, ns, firelist, sfirelist,witness))
    {
        // We know that !psi holds, so !AG(psi) holds by definition. We
        // hence found a witness for !AG(psi) and return.
    ...
    while (true)
    {
        ...
        if (currentFirelistIndex--)
        {
            ...
            if (newCachedResult == UNKNOWN)
            {
                ...
                //==========================================
                // (6a) quick check 1: !psi => !AG(psi)
                //==========================================

                // check psi
                if (!psi->check(s, ns, firelist, sfirelist, witness))
                {
                    // We know that !psi holds, so !AG(psi) holds by
                    // definition. We hence found a witness for !AG(psi) and
    ...
    myfirelist.visible = visible;
		...
		if(dfs > firedAll)
            ...
            {
                //===========================================================
                // (8a) we did not found a witness -> AG(psi) is true
                //===========================================================
    ...
    // => AG(psi) does not hold - we only need to organize the witness
    ...
    // AG(psi) does not hold and we found a witness path
    return false;
}

bool AGFormula::checkfair(Store<void *> &s, NetState &ns, Firelist &firelist,
                      std::vector<int> &witness)
{
    //==================================
    // (1) prepare marking and payload
...
FormulaInfo *AGFormula::getInfo() const
{
...
int AGFormula::countSubFormulas() const
{
    return 1 + psi->countSubFormulas();
}

void AGFormula::print()
{
	std::cout << "AG( ";
	psi -> print();
	std::cout << ")";
