...
*/
DisjunctionFormula::DisjunctionFormula(Petrinet * n, std::vector<CTLFormula *> _subs)
                               ...
                               Firelist &firelist, Firelist &sfirelist,std::vector<int> &witness)
{
    ...
    for (arrayindex_t i = 0; i < cardSubs; i++)
    {
        if (subs[i]->check(s, ns, firelist, sfirelist,witness))
        {
...
void DisjunctionFormula::setVisible()
{
