...
\brief Definition of the CTL AR operator.
*/
...
class ARFormula : public DFSFormula
{
public:
    ARFormula(Petrinet *, CTLFormula *phi, CTLFormula *psi);
	...
	std::string ctl2string() { return std::string("A(") + phi -> ctl2string() + std::string(" R ") + psi -> ctl2string() + std::string(")");}
...
private:
    ...
    /// check whether A(phi R psi) holds
    ...
    /// the first inner formula for A(phi R psi)
    CTLFormula *phi;
    /// the second inner formula for A(phi R psi)
    CTLFormula *psi;
    virtual void print();
    void setVisible();
};
