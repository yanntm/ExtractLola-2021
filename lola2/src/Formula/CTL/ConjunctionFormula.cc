...
*/
ConjunctionFormula::ConjunctionFormula(Petrinet * n, std::vector<CTLFormula *> _subs)
...
bool ConjunctionFormula::check(Store<void *> &s, NetState &ns,
                               Firelist &firelist, Firelist &sfirelist, std::vector<int> &witness)
{
    // check every subformula
    for (arrayindex_t i = 0; i < cardSubs; i++)
    {
        if (not subs[i]->check(s, ns, firelist, sfirelist, witness))
        {
...
void ConjunctionFormula::setVisible()
{
	...
	for(arrayindex_t i = 0; i < cardSubs;i++)
	{
		...
		{
			visible[j] = visible[j] || subs[i]->visible[j];
		}
	}
