...
{
public:
    explicit ConjunctionFormula(Petrinet *, std::vector<CTLFormula *>);
    ...
    std::string ctl2string() 
    {
	bool needcomma = false;
	std::string result = std::string("");
	for(int i = 0; i < cardSubs;i++)
	{
		if(needcomma) result += std::string(" AND ");
		needcomma = true;
		result += subs[i]->ctl2string();
	}
	return result;
    }
