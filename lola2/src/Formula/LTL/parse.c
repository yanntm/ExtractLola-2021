...
bin_simpler(tl_Node *ptr)
{
    if (ptr)
        ...
        case U_OPER:
            ...
            if (ptr->lft->ntyp == TTRUE &&
                    ptr->rgt->ntyp == V_OPER &&
                    ptr->rgt->lft->ntyp == FFALSE &&
                    ptr->rgt->rgt->ntyp == U_OPER &&
                    ptr->rgt->rgt->lft->ntyp == TTRUE)
            {
                ptr = ptr->rgt;
                break;
            }
        ...
        case V_OPER:
            ...
            if (ptr->lft->ntyp == FFALSE &&
                    ptr->rgt->ntyp == U_OPER &&
                    ptr->rgt->lft->ntyp == TTRUE &&
                    ptr->rgt->rgt->ntyp == V_OPER &&
                    ptr->rgt->rgt->lft->ntyp == FFALSE)
            {
                ptr = ptr->rgt;
                break;
            }
        ...
        case NEXT:
            ...
            if (ptr->lft->ntyp == V_OPER &&
                    ptr->lft->lft->ntyp == FFALSE &&
                    ptr->lft->rgt->ntyp == U_OPER &&
                    ptr->lft->rgt->lft->ntyp == TTRUE)
            {
                ptr = ptr->lft;
                break;
            }
            /* NEW : X F G p == F G p */
            if (ptr->lft->ntyp == U_OPER &&
                    ptr->lft->lft->ntyp == TTRUE &&
                    ptr->lft->rgt->ntyp == V_OPER &&
                    ptr->lft->rgt->lft->ntyp == FFALSE)
            {
                ptr = ptr->lft;
                break;
            }
        ...
        case AND:
            ...
            if (isequal(ptr->lft, ptr->rgt)	/* (p && p) == p */
                    ||  ptr->rgt->ntyp == FFALSE	/* (p && F) == F */
                    ||  ptr->lft->ntyp == TTRUE	/* (T && p) == p */
                    ||  implies(ptr->rgt, ptr->lft))/* NEW */
            {
                ptr = ptr->rgt;
                break;
            }
            if (ptr->rgt->ntyp == TTRUE	/* (p && T) == p */
                    ||  ptr->lft->ntyp == FFALSE	/* (F && p) == F */
        ...
        case OR:
            ...
            if (isequal(ptr->lft, ptr->rgt)	/* (p || p) == p */
                    ||  ptr->rgt->ntyp == FFALSE	/* (p || F) == p */
                    ||  ptr->lft->ntyp == TTRUE	/* (T || p) == T */
                    ||  implies(ptr->rgt, ptr->lft))/* NEW */
            {
                ptr = ptr->lft;
                break;
            }
            if (ptr->rgt->ntyp == TTRUE	/* (p || T) == T */
                    ||  ptr->lft->ntyp == FFALSE	/* (F || p) == p */
                    ||  implies(ptr->lft, ptr->rgt))/* NEW */
            {
                ptr = ptr->rgt;
                break;
