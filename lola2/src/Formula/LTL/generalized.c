...
int *fin, *final, rrank, scc_id, scc_size, *bad_scc;
...
int gdfs(GState *s)
{
    ...
    for (t = s->trans->nxt; t != s->trans; t = t->nxt)
    {
        ...
        {
            for (c = sscc_stack->nxt; c != 0; c = c->nxt)
                if (c->gstate == t->to)
                {
    ...
    {
        while (sscc_stack != scc)
        {
            sscc_stack->gstate->incoming = scc_id;
            sscc_stack = sscc_stack->nxt;
        }
