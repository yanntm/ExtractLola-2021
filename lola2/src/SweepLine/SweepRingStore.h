...
Petrinet * net;
    ...
    SweepRingStore(arrayindex_t _size, arrayindex_t _front_offset, arrayindex_t _transient_offset,
                   FullCopyEncoder *fce, NetStateEncoder *nse, arrayindex_t _nr_of_threads,
                   hash_t _nr_of_buckets,Petrinet * n, Mara * mem);
    /// destructor
    ~SweepRingStore();

    /// check if a state is in the store at progress offset, insert it if not
    bool searchAndInsert(NetState &ns, int offset, T **payload, arrayindex_t thread);
    ...
    int getNumberOfDeletedStates();

    VectorStore<T> *createSweepStore(Petrinet *, Mara *);
